...
const FAQItem = ({ question, answer }) => {
  ...
  useEffect(() => {
    if (answerRef.current) {
      if (isOpen) {
        const height = answerRef.current.scrollHeight; // Получаем высоту содержимого
        // Добавляем запас (50px) для учета padding и возможных расхождений
        answerRef.current.style.maxHeight = `${height + 50}px`;
      } else {
        answerRef.current.style.maxHeight = '0px'; // Сбрасываем при закрытии
      }
    }
  }, [isOpen]);
